--서브쿼리
--SELECT문이 SELECT구문으로 들어가는 형태: 그칼라 서브쿼리
...
--주의할점, 단일행 이여야 합니다.컬럼값도 1개여야합니다.
SELECT *
FROM EMPLOYEES
WHERE JOB_ID = (SELECT * FROM EMPLOYEES WHERE EMPLOYEE_ID = 103); --안됨

SELECT *
FROM EMPLOYEES
WHERE JOB_ID > (SELECT JOB_ID FROM EMPLOYEES WHERE EMPLOYEE_ID = 103 OR EMPLOYEE_ID = 104);--안됨
--ERR
---------------------------------------------------------
...
--급여가 4800보다 큰 사람들 반대로는 9500보다 작은 사람들

--ALL 최대값 보다 큼, 최소값 보다 작음
SELECT *
FROM EMPLOYEES
WHERE SALARY  < ALL (SELECT SALARY FROM EMPLOYEES WHERE FIRST_NAME='David');
--급여가 9500보다 큰 사람들 반대로는 4800보다 작은 사람들

--직업이 IT_PROG 인 사람들의 최소값 보다 큰 급여를 받는 사람들
...
-------------------------------------------------
--스칼라 서브쿼리
--JOIN시에 특정테이블의 1컬럼을 가지고 올때 유리합니달
SELECT FIRST_NAME,
       EMAIL,
...
SELECT E.FIRST_NAME,
       ...
       (SELECT DEPARTMENT_NAME FROM DEPARTMENTS D WHERE D.DEPARTMENT_ID = E.DEPARTMENT_ID) AS DEPARTMENT_ID
FROM EMPLOYEES E;
...
--ROWNUM은 조회된 순서이기 때문에 ORDER와 같이 사용되면 ROWNUM섞이는 문제 
--문법1
SELECT FIRST_NAME,
       SALARY,
       ROWNUM
...
--ROWNUM는 무조건 1번째부터 조회가 가능하기 때문에 11~20하면 안나옴
SELECT FIRST_NAME,
       SALARY,
       ROWNUM
FROM(SELECT *
...
--스칼리쿼리 -LEFT JOIN과 같은 역할,한번에 1개의 컬럼을 가져올때
